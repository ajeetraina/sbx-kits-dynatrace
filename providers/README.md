...
| [local](./local.md) **(deprecated)** | Self-hosted in the sandbox (`@dynatrace-oss/dynatrace-mcp-server`, upstream deprecated - final `2.1.2`) | SaaS (`*.apps.dynatrace.com`) | platform token via `sbx secret set-custom` | proxy injects `Authorization: Bearer` on the wire |
...
server still runs and ships as the `local` kit, but it is **deprecated upstream**
(`2.1.2` was the final release), so `remote` is the path to use. Both target
Dynatrace **SaaS** (the Gen3 `*.apps.dynatrace.com`
